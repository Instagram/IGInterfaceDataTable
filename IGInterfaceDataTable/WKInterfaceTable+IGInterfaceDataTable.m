...
- (BOOL)updating {
    NSNumber *number = objc_getAssociatedObject(self, @selector(updating));
    return [number boolValue];
}

- (void)setUpdating:(BOOL)updating {
    NSNumber *number = [NSNumber numberWithBool: updating];
    objc_setAssociatedObject(self, @selector(updating), number, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
- (void)endUpdates {
    ...
    [self.rowIndexesToAdd sortUsingComparator:^NSComparisonResult(id obj1, id obj2) {
    ...
    [self.rowIndexesToAdd enumerateObjectsUsingBlock:^(NSIndexPath *indexPath, NSUInteger idx, BOOL *stop) {
        [self insertRowsAtIndexPaths:@[indexPath] withRowType:[self.rowTypesToAdd objectForKey:indexPath]];
    }];
